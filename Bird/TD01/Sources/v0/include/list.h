...
#define list_for_each_safe(pos, head) \
	for (struct list_head *pos = (head)->next, *_n = pos->next; pos != (head); pos = _n, _n = pos->next)
...
#define list_for_each_entry_safe(type, pos, head, member)		\
	for (type *pos = list_entry((head)->next, type, member), *_n = list_entry(pos->member.next, type, member);	\
	     &pos->member != (head); 					\
